...
//
// A native <table role="grid"> for a task/timeline planning grid. The
// consumer supplies rows/cells as light-DOM children — this catalog does
// not implement the table sub-element families, so the consumer's markup
// is plain HTML `<thead>`/`<tbody>`/`<tr>`/`<th>`/`<td>` exactly as it
// would be for a native `<table>`.
//
// Attributes:
//   label — REQUIRED. Accessible name describing the Gantt chart, via
//     aria-label.
//
...
//
// References:
//   - components/gantt-table/index.md (canonical contract)

import { moveChildrenInto, passThroughAttributes, rootClassName } from "../lib/dom-utils.js";

const HANDLED = new Set(["label"]);

export class GanttTable extends HTMLElement {
    connectedCallback(): void {
        if (this.querySelector(":scope > table.gantt-table")) return;

        const table = document.createElement("table");
        table.className = rootClassName(this, "gantt-table");
